...
export function registerGenerateCommand(program: Command): void {
  program
    ...
    .action((template: string, name: string, options: { port?: number; entity?: string; out?: string }) =>
      withTelemetry("generate:template", async () => {
        const result = await withSpinner("Rendering APPNEURAL template", () =>
        ...
        logger.success(
          `APPNEURAL template '${result.template}' created ${result.fileCount} files at ${result.outputDir}`
        );
      })
    );
}
